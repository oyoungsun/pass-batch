...
public class AddPassesTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        //이용권 시작 일시 1일 전 user group 내 각 사용자에게 이용권을 추가할 것이다.
        final LocalDateTime startedAt = LocalDateTime.now().minusDays(1);
        //아직 처리 안되었고, -1일 보다 지난 시점인 녀석들 다 가져옴
        ...
        for(BulkPassEntity bulkPassEntity: bulkPassEntities){
            ...
            //user그룹마다 user들의 id를 가져옴.
            count += addPasses(bulkPassEntity, userIds);
            bulkPassEntity.setStatus(BulkPassStatus.COMPLETED); //상태변경

        }

        //대량 이용권 정보를 돌면서 user group에 속한 userId를 조회하고 해당 user로 이용권을 추가한다.
        log.info("AddPassesTasklet - execute: 이용권 {}건 추가 완료 startedAt={}", count, startedAt);

        return RepeatStatus.FINISHED;
    }

    //bulk엔티티의 정보로 pass데이터를 생성한다.
